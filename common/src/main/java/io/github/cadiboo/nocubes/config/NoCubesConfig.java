	...
	public static class Smoothables {
		...
		static void updateUserDefinedSmoothableStringLists(boolean newValue, BlockState[] states, List<String> whitelist, List<String> blacklist) {
			...
			for (var state : states) {
				var string = serializer.toString(state);
		...
		static void recomputeInMemoryLookup(IBlockStateSerializer serializer, Collection<Block> blockRegistry, List<? extends String> whitelist, List<? extends String> blacklist, boolean useDefaultSmoothables) {
			LOG.debug("Recomputing in-memory smoothable lookups from user-defined smoothable string lists");
			var whitelisted = parseBlockStates(serializer, whitelist);
			var blacklisted = parseBlockStates(serializer, blacklist);
			blockRegistry.parallelStream()
		...
		static Set<BlockState> parseBlockStates(IBlockStateSerializer serializer, List<? extends String> list) {
			var set = Sets.<BlockState>newIdentityHashSet();
			list.stream()
				.map(serializer::fromStringOrNull)
