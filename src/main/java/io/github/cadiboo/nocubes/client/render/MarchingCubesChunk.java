 ...
 * @author Cadiboo
 */
public final class MarchingCubesChunk {

	private static final MethodHandle RenderChunk_preRenderBlocks_HANDLE;
	static {
		try {
...
//						edges[i] = vertices.size();
						Vec3 nv = new Vec3();
						int[] e = EDGE_INDEX[i];
						int[] p0 = CUBE_VERTICES[e[0]];
						int[] p1 = CUBE_VERTICES[e[1]];
						float a = grid[e[0]];
						float b = grid[e[1]];
						float d = a - b;
						float t = 0;
...
//					}
		...
		final RegionRenderCacheBuilder regionRenderCacheBuilder = generator.getRegionRenderCacheBuilder();

		for (int i = 0; i < vertices.size(); i += 4) {
			final Vec3 v0 = vertices.get(i);
			final Vec3 v1 = vertices.get(i + 1);
			final Vec3 v2 = vertices.get(i + 2);
			final Vec3 v3 = vertices.get(i + 3);

			final MutableBlockPos pos = new MutableBlockPos(
					(int) MathHelper.average(new long[]{(long) v0.xCoord, (long) v1.yCoord, (long) v2.yCoord, (long) v3.yCoord}),
					(int) MathHelper.average(new long[]{(long) v0.xCoord, (long) v1.yCoord, (long) v2.yCoord, (long) v3.yCoord}),
					(int) MathHelper.average(new long[]{(long) v0.xCoord, (long) v1.yCoord, (long) v2.yCoord, (long) v3.yCoord})
			);
			...
			for (final BlockRenderLayer blockRenderLayer : BlockRenderLayer.values()) {
				...
				if (!RenderChunkRebuildChunkHooksHooks.canBlockRenderInLayer(renderChunk, cache, event.getGenerator(), compiledchunk, blockRendererDispatcher, renderChunkPos, new VisGraph(), pos, block, state, blockRenderLayer))
					continue;

				final BufferBuilder bufferBuilder = regionRenderCacheBuilder.getWorldRendererByLayerId(blockRenderLayer.ordinal());
