...
//			final boolean shouldRenderEast = !eastFluidState.getFluid().isEquivalentTo(fluid);
			...
			shouldRenderDown &= !(Config.renderSmoothTerrain && TERRAIN.test(worldIn.getBlockState(pooledMutableBlockPos.setPos(renderX, renderY - 1, renderZ))));
			boolean shouldRenderUp = state.shouldSideBeRendered(worldIn, renderPos, EnumFacing.UP);
			shouldRenderUp &= !(Config.renderSmoothTerrain && TERRAIN.test(worldIn.getBlockState(pooledMutableBlockPos.setPos(renderX, renderY + 1, renderZ))));
			boolean shouldRenderNorth = state.shouldSideBeRendered(worldIn, renderPos, EnumFacing.NORTH);
			shouldRenderNorth &= !(Config.renderSmoothTerrain && TERRAIN.test(worldIn.getBlockState(pooledMutableBlockPos.setPos(renderX, renderY, renderZ - 1))));
			boolean shouldRenderSouth = state.shouldSideBeRendered(worldIn, renderPos, EnumFacing.SOUTH);
			shouldRenderSouth &= !(Config.renderSmoothTerrain && TERRAIN.test(worldIn.getBlockState(pooledMutableBlockPos.setPos(renderX, renderY, renderZ + 1))));
			boolean shouldRenderWest = state.shouldSideBeRendered(worldIn, renderPos, EnumFacing.WEST);
			shouldRenderWest &= !(Config.renderSmoothTerrain && TERRAIN.test(worldIn.getBlockState(pooledMutableBlockPos.setPos(renderX - 1, renderY, renderZ))));
			boolean shouldRenderEast = state.shouldSideBeRendered(worldIn, renderPos, EnumFacing.EAST);
			shouldRenderEast &= !(Config.renderSmoothTerrain && TERRAIN.test(worldIn.getBlockState(pooledMutableBlockPos.setPos(renderX + 1, renderY, renderZ))));
