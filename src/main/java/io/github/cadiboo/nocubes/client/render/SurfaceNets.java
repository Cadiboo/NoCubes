...
import io.github.cadiboo.nocubes.client.SurfaceNetsUtil;
...
import static io.github.cadiboo.nocubes.client.SurfaceNetsUtil.DENSITY_CACHE_SIZE_X;
import static io.github.cadiboo.nocubes.client.SurfaceNetsUtil.DENSITY_CACHE_SIZE_Y;
...
public final class SurfaceNets {
	...
	public static void renderPre(final RebuildChunkPreEvent event) {
		...
		try {
			...
			//<editor-fold desc="Field to Local Variable Conversion">
			final int maxZ = MESH_SIZE_Z;
			final int maxY = MESH_SIZE_Y;
			final int maxX = MESH_SIZE_X;
			final int[] EDGE_TABLE = SurfaceNetsUtil.EDGE_TABLE;
			final int[] CUBE_EDGES = SurfaceNetsUtil.CUBE_EDGES;
			//</editor-fold>

			//<editor-fold desc="Setup Surface Nets">
						...
						for (int z = 0; z < 2; ++z) {
							for (int y = 0; y < 2; ++y) {
								for (int x = 0; x < 2; ++pointIndex) {
									...
									final float p = densityCache[(currentPos[0] + x) + DENSITY_CACHE_SIZE_X * ((currentPos[1] + y) + DENSITY_CACHE_SIZE_Y * (currentPos[2] + z))];
...
//						int mask = SurfaceNetsUtil.calculateNeighbourDensitiesAndMask(grid, currentPos[0], currentPos[1],currentPos[2] , densityCache);
						...
						if (ModConfig.approximateLightingLevel != OFF) {
							getBrightnessPos:
							for (int y = -1; y < 2; ++y) {
								for (int z = -2; z < 3; ++z) {
									for (int x = -1; x < 2; ++x) {
...
//					final float s = 1.0F / e_count;
						final float s = isoSurfaceLevel / e_count;
						for (int i = 0; i < 3; ++i) {
...
//						}
	...
	public static void renderPost(final RebuildChunkPostEvent event) {
		...
		USED_BLOCK_RENDER_LAYERS_THREAD_LOCAL.remove();
