...
//		return (skyLight << 16) | blockLight;
	...
	public int get(int x, int y, int z) {
		...
		if (index >= array.length) // TODO: Remove
			return 0;

		int color = array[index];
		if (color == -1) {
			BlockPos.Mutable unsetWorldPos = this.mutablePos;
			BlockPos start = area.start;
			unsetWorldPos.set(start.getX() + x, start.getY() + y, start.getZ() + z);
	...
	private int compute(int index, BlockPos.Mutable worldPos) {
		Area area = this.area;
		int light = area.getAndCacheBlocks()[index].getLightValue(area.world, worldPos);
		return DynamicLights.getCombinedLight(worldPos, light);
	}
