...
public class RenderDispatcher {
	...
	public static void renderChunk(final RebuildChunkPreRenderEvent event) {
		...
		NoCubes.getProfiler().start("dispatch renderChunk");
	...
	private static void renderChunk(
		...
		NoCubes.getProfiler().start("renderChunk mesh and extended liquids");
		final boolean[] usedBlockRenderLayers = USED_RENDER_LAYERS.get();
		USED_RENDER_LAYERS_SET.set(false);
		final BlockRendererDispatcher blockRendererDispatcher = Minecraft.getInstance().getBlockRendererDispatcher();

		{
			NoCubes.getProfiler().start("setup renderChunk mesh caches");
//			for(MeshLayer layer : meshLayers)
			...
			NoCubes.getProfiler().end();
			NoCubes.getProfiler().start("renderMesh");
			try {
		...
		if (NoCubesConfig.extendLiquids != ExtendLiquidRange.Off) {
			...
			NoCubes.getProfiler().end();
			NoCubes.getProfiler().start("renderChunk extended liquids");
	...
	private static StateCache generateMeshAndLightStateCache(
		...
		NoCubes.getProfiler().start("generateMeshAndLightStateCache");
		try {
	...
	private static StateCache generateExtendedWaterStateCache(
		...
		NoCubes.getProfiler().start("generateExtendedWaterStateCache");
		try {
	...
	public static void renderBlock(final RebuildChunkBlockEvent event) {
		try {
			if (!USED_RENDER_LAYERS_SET.get()) {
				final boolean[] usedRenderLayers = USED_RENDER_LAYERS.get();
				for (int ordinal = 0; ordinal < BlockRenderLayer.values().length; ++ordinal) {
					event.getUsedBlockRenderLayers()[ordinal] |= usedRenderLayers[ordinal];
				}
				USED_RENDER_LAYERS_SET.set(true);
				//reset so it gets re-initialised so that we don't get ghost blocks from now-unused render layers
				Arrays.fill(usedRenderLayers, false);
			}
