...
public final class SmoothLightingFluidBlockRenderer extends BlockFluidRenderer {
	...
	public boolean renderFluid(final IBlockAccess worldIn, final IBlockState state, final BlockPos pos, final BufferBuilder buffer) {
		...
		try {
			...
			shouldRenderDown &= !(NoCubesConfig.Client.render && NoCubes.smoothableHandler.isSmoothable(worldIn.getBlockState(pooledMutableBlockPos.setPos(x, y - 1, z))));
			boolean shouldRenderUp = state.shouldSideBeRendered(worldIn, pos, EnumFacing.UP);
			shouldRenderUp &= !(NoCubesConfig.Client.render && NoCubes.smoothableHandler.isSmoothable(worldIn.getBlockState(pooledMutableBlockPos.setPos(x, y + 1, z))));
			boolean shouldRenderNorth = state.shouldSideBeRendered(worldIn, pos, EnumFacing.NORTH);
			shouldRenderNorth &= !(NoCubesConfig.Client.render && NoCubes.smoothableHandler.isSmoothable(worldIn.getBlockState(pooledMutableBlockPos.setPos(x, y, z - 1))));
			boolean shouldRenderSouth = state.shouldSideBeRendered(worldIn, pos, EnumFacing.SOUTH);
			shouldRenderSouth &= !(NoCubesConfig.Client.render && NoCubes.smoothableHandler.isSmoothable(worldIn.getBlockState(pooledMutableBlockPos.setPos(x, y, z + 1))));
			boolean shouldRenderWest = state.shouldSideBeRendered(worldIn, pos, EnumFacing.WEST);
			shouldRenderWest &= !(NoCubesConfig.Client.render && NoCubes.smoothableHandler.isSmoothable(worldIn.getBlockState(pooledMutableBlockPos.setPos(x - 1, y, z))));
			boolean shouldRenderEast = state.shouldSideBeRendered(worldIn, pos, EnumFacing.EAST);
			shouldRenderEast &= !(NoCubesConfig.Client.render && NoCubes.smoothableHandler.isSmoothable(worldIn.getBlockState(pooledMutableBlockPos.setPos(x + 1, y, z))));
...
//				pooledMutableBlockPos.setPos(posX - (j & 1), posY, posZ - (j >> 1 & 1));
	...
	public boolean smoothLighting() {
		return true;//Config.smoothFluidLighting;
	}

	public boolean colors() {
		return true;//Config.smoothFluidColors;
	}

	public boolean textures() {
		return true;//Config.naturalFluidTextures;
	}
