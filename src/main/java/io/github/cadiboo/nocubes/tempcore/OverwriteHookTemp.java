...
import io.github.cadiboo.rcrch.hooks.RenderChunkRebuildChunkHooksHooks;
...
import static io.github.cadiboo.rcrch.util.Utils.compiledChunk_setLayerUsed;
import static io.github.cadiboo.rcrch.util.Utils.renderChunk_postRenderBlocks;
import static io.github.cadiboo.rcrch.util.Utils.renderChunk_preRenderBlocks;
...
//			float x, float y, float z, ChunkCompileTaskGenerator generator
//	) {
...
//			}
//
//			ChunkCache lvt_10_1_ = renderChunk.createRegionRenderCache(world, blockpos.add(-1, -1, -1), blockpos.add(16, 16, 16), 1);
...
//	}
	...
	public static void rebuildChunk(
		...
		CompiledChunk compiledchunk = new CompiledChunk();
		int i = 1;
		BlockPos blockpos = renderChunk.position;
		BlockPos blockpos1 = blockpos.add(15, 15, 15);
		generator.getLock().lock();
		...
		if (!renderChunk.worldView.isEmpty())
		{
			++RenderChunk.renderChunksUpdated;
			boolean[] aboolean = new boolean[BlockRenderLayer.values().length];
			BlockRendererDispatcher blockrendererdispatcher = Minecraft.getMinecraft().getBlockRendererDispatcher();
			...
			for (BlockPos.MutableBlockPos blockpos$mutableblockpos : BlockPos.getAllInBoxMutable(blockpos, blockpos1))
			{
				IBlockState iblockstate = renderChunk.worldView.getBlockState(blockpos$mutableblockpos);
				Block block = iblockstate.getBlock();
				...
				{
					TileEntity tileentity = renderChunk.worldView.getTileEntity(blockpos$mutableblockpos, Chunk.EnumCreateEntityType.CHECK);
		...
		try
		{
			...
			renderChunk.setTileEntities.clear();
			renderChunk.setTileEntities.addAll(lvt_10_1_);
			renderChunk.renderGlobal.updateTileEntities(set1, set);
		}
		finally
		{
