...
//		final Face face = Face.of();
			...
			for (int y = 0; y < meshSizeY; ++y, ++n, m += 2) {
				for (int x = 0; x < meshSizeX; ++x, ++n, ++m) {
					...
					for (int i = 0; i < 3; ++i) {
						...
						boolean done = !action.apply(pos, face, normal, averageNormal, normalDirection);
						if (done) {
							close(new Vec[0], face, normal, averageNormal);
							return;
						}
					}
				}
		...
		close(new Vec[0], null, normal, averageNormal);
	}
...
//	}
	...
	public interface MeshAction {
		...
		boolean apply(BlockPos.Mutable pos, Face face, Face normal, Vec averageNormal, Direction direction);
