...
abstract class SDFMesher implements Mesher {
	...
	protected static float[] generateDistanceField(Area area, Predicate<IBlockState> isSmoothable, boolean smoother, @Nullable TestMesh testMesh) {
	...
	private static float[] generateDistanceField(Area area, Predicate<IBlockState> isSmoothable, boolean smoother) {
	...
	private static float[] generateDistanceField(Area area, Predicate<IBlockState> isSmoothable) {
		...
		float[] distanceField = FIELD.takeArray(distanceFieldSize);
	...
	static float[] generateNegativeDensityField(Area area, Predicate<IBlockState> isSmoothable) {
		IBlockState[] states = area.getAndCacheBlocks();
		int length = area.numBlocks();
		float[] densityField = FIELD.takeArray(length);
		for (int i = 0; i < length; ++i)
