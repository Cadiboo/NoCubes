...
public final class MarchingCubes implements MeshGenerator {
	...
	/**
	 * The points in CUBE_VERTS that are the start and end points of the edge
	 */
			...
			for (x[1] = 0; x[1] < dims[1] - 1; ++x[1], ++n)
				for (x[0] = 0; x[0] < dims[0] - 1; ++x[0], ++n) {
					...
					for (byte i = 0; i < trianglePoints.length; i += 3) {
						faces.add(
								Face.retain(
										Vec3.retain(vertices.get(edges2vertices[trianglePoints[i]])),
										Vec3.retain(vertices.get(edges2vertices[trianglePoints[i + 1]])),
										Vec3.retain(vertices.get(edges2vertices[trianglePoints[i + 2]]))
								)
						);
					}
