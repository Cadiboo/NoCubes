...
public class SurfaceNets2 {
	...
	public static synchronized void renderPre(final RebuildChunkPreEvent event) {
		...
		synchronized (positions) {
			positions.put(rcp, 0);
			LOGGER.info("pre  : " + rcp + " | " + positions.get(rcp));
		}
	}

	public static synchronized void renderBlock(final RebuildChunkBlockEvent event) {
		final BlockPos rcp = event.getRenderChunkPosition().toImmutable();
		synchronized (positions) {
			positions.put(rcp, positions.get(rcp) + 1);
			if (event.getBlockPos().equals(rcp)) {
				LOGGER.info("block: " + rcp + " | " + positions.get(rcp));
			}
		}
	}

	public static synchronized void renderPost(final RebuildChunkPostEvent event) {
		final BlockPos rcp = event.getRenderChunkPosition().toImmutable();
		synchronized (positions) {
			LOGGER.info("post : " + rcp + " | " + positions.get(rcp));
			positions.remove(rcp);
		}
	}
...
//
//		LOGGER.info("starting" + renderChunkPos);
//
//		// The shadow values.
...
//
//		synchronized (renderChunkPosConcurrentHashMap) {
//
...
//		}
//		LOGGER.info("finished" + renderChunkPos);
//
//	}
//
//	public static void renderLayer(final RebuildChunkBlockRenderInLayerEvent event) {
...
//			final BlockPos renderChunkPos = event.getRenderChunkPosition();
//
//			if (pos.equals(renderChunkPos)) {
//				LOGGER.info("rendering all for " + renderChunkPos);
//			}
//
...
//				LOGGER.warn(renderChunkPos + " | " + npe);
//			}
//
//			if (cancel) {
...
//
//			try {
//				LOGGER.error("removing" + renderChunkPos);
