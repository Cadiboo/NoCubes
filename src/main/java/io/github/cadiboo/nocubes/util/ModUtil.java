...
//	private static final Field configManager_CONFIGS = ReflectionUtil.getFieldOrCrash(ConfigManager.class, "CONFIGS");
	 ...
	 */
	public static boolean shouldSmooth(final BlockState state) {
	 ...
	 * @return If the state should be smoothed
	 */
	public static boolean shouldSmoothLeaves(final BlockState state) {
		return ModConfig.smoothLeavesSeparate && state.getBlock() instanceof LeavesBlock;
	}

	/**
	 * @return negative density if the block is smoothable (inside the isosurface), positive if it isn't
	 */
	public static float getIndividualBlockDensity(final boolean shouldSmooth, final BlockState state, final BlockView cache, final BlockPos pos) {
		float density = 0;

		if (shouldSmooth) {
			final BoundingBox box = state.getCollisionShape(cache, pos).getBoundingBox();
		...
		} else if (/*ModConfig.debug.connectToNormal && */(state.isSimpleFullBlock(cache, pos))) {
			// OK OK OK OK OK LordPhrozen, I've done it (kinda)
			density += (float) ModConfig.smoothOtherBlocksAmount;
		} else if (state.getMaterial() == Material.UNUSED_PLANT) {
			density -= 0.75;
		} else {
