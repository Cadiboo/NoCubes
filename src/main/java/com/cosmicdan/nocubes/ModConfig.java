...
public class ModConfig {
	...
	public static final HashSet<Integer>	SMOOTHBLOCKS_IDS	= new HashSet<>();
	...
	public static void doConfig(File configPath) {
		...
		final Property MOD_ENABLED_PROP = getEnabled(MOD_ENABLED);
		final Property AUTOSTEPUP_ENABLED_PROP = getStepup(AUTOSTEPUP_ENABLED);

		// build the default blocklist
		SMOOTHBLOCKS = getDefaultBlocks();
		final Property SMOOTHBLOCKS_PROP = CONFIG.get("smoothing", "smoothblocks", SMOOTHBLOCKS);
		SMOOTHBLOCKS_PROP.setComment("The list of blocks to be smoothed, in modid:name format");
	...
	// B:smoothing
	public static Property getEnabled(final boolean defaultVal) {
		final Property PROP = CONFIG.get("general", "smoothing", defaultVal);
		PROP.setComment("Set if block smoothing is enabled by default. This can be toggled in-game.");
		return PROP;
	}

	public static void setEnabled(final boolean newVal) {
		getEnabled(newVal).set(newVal);
		MOD_ENABLED = newVal;
	}

	// B:stepup
	public static Property getStepup(final boolean defaultVal) {
		final Property PROP = CONFIG.get("general", "stepup", defaultVal);
		// PROP.comment = "Set if step-up is enabled by default. This can be toggled in-game.";
		PROP.setComment("Not yet implemented. Use Iguana Tweaks for now if you want this.");
		return PROP;
	}

	public static void setStepup(final boolean newVal) {
		getStepup(newVal).set(newVal);
		AUTOSTEPUP_ENABLED = newVal;
	}

	private static String[] getDefaultBlocks() {
		final ArrayList<String> SMOOTHBLOCKS_NAMES = new ArrayList<>();
	...
	public static void buildSmoothBlockIds() {
		...
		for (final String smoothBlocksListEntry : SMOOTHBLOCKS) {
			final String[] blockFqn = smoothBlocksListEntry.split(":");
			if ((blockFqn[0] == null) || (blockFqn[1] == null)) {
				Main.LOGGER.warn(" - Invalid block entry in config: " + smoothBlocksListEntry);
				continue;
			}
			final Block foundBlock = GameRegistry.findBlock(blockFqn[0], blockFqn[1]);
			if (foundBlock == null) {
