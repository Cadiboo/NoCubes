...
public class ModRenderGlobal extends RenderGlobal {
	...
	public Set<ModRenderChunk>										chunksToUpdate					= Sets.<ModRenderChunk>newLinkedHashSet();
	/** List of OpenGL lists for the current render pass */
	public List<ModRenderGlobal.ContainerLocalRenderInformation>	renderInfos						= Lists.<ModRenderGlobal.ContainerLocalRenderInformation>newArrayListWithCapacity(69696);
	...
	public ModViewFrustum											viewFrustum;
	...
	public ModRenderList											renderContainer;
	...
	public ModRenderGlobal(final Minecraft mcIn) {
		super(mcIn);
		this.mc = mcIn;
		...
		if (this.vboEnabled) {
			this.renderContainer = new ModVboRenderList();
			this.renderChunkFactory = new VboChunkFactory();
		} else {
			this.renderContainer = new ModRenderList();
	...
	public void renderSky(final BufferBuilder bufferBuilderIn, final float posY, final boolean reverseX) {
		final int i = 64;
		final int j = 6;
	...
	public void renderStars(final BufferBuilder bufferBuilderIn) {
	...
	public void loadRenderers() {
		if (this.world != null) {
			...
			this.viewFrustum = new ModViewFrustum(this.world, this.mc.gameSettings.renderDistanceChunks, this, this.renderChunkFactory);
	...
	public void renderEntities(final Entity renderViewEntity, final ICamera camera, final float partialTicks) {
		...
		if (this.renderEntitiesStartupCounter > 0) {
			...
			for (int i = 0; i < this.world.weatherEffects.size(); ++i) {
				final Entity entity1 = this.world.weatherEffects.get(i);
			...
			final List<Entity> list1 = Lists.<Entity>newArrayList();
			final List<Entity> list2 = Lists.<Entity>newArrayList();
			final BlockPos.PooledMutableBlockPos blockpos$pooledmutableblockpos = BlockPos.PooledMutableBlockPos.retain();

			for (final ModRenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation : this.renderInfos) {
				final Chunk chunk = this.world.getChunkFromBlockCoords(renderglobal$containerlocalrenderinformation.renderChunk.getPosition());
				...
				if (!classinheritancemultimap.isEmpty()) {
					for (final Entity entity2 : classinheritancemultimap) {
						if (!entity2.shouldRenderInPass(pass)) {
							continue;
						}
						final boolean flag = this.renderManager.shouldRender(entity2, camera, d0, d1, d2) || entity2.isRidingOrBeingRiddenBy(this.mc.player);

						if (flag) {
							final boolean flag1 = this.mc.getRenderViewEntity() instanceof EntityLivingBase ? ((EntityLivingBase) this.mc.getRenderViewEntity()).isPlayerSleeping() : false;
			...
			for (final ModRenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation1 : this.renderInfos) {
				final List<TileEntity> list3 = renderglobal$containerlocalrenderinformation1.renderChunk.getCompiledChunk().getTileEntities();

				if (!list3.isEmpty()) {
					for (final TileEntity tileentity2 : list3) {
			...
			for (final DestroyBlockProgress destroyblockprogress : this.damagedBlocks.values()) {
				...
				if (this.world.getBlockState(blockpos).getBlock().hasTileEntity()) {
					...
					if (tileentity1 instanceof TileEntityChest) {
						final TileEntityChest tileentitychest = (TileEntityChest) tileentity1;
	 ...
	 */
	public boolean isOutlineActive(final Entity entityIn, final Entity viewer, final ICamera camera) {
	...
	public int getRenderedChunks() {
		...
		for (final ModRenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation : this.renderInfos) {
			final ModCompiledChunk compiledchunk = renderglobal$containerlocalrenderinformation.renderChunk.compiledChunk;

			if ((compiledchunk != ModCompiledChunk.DUMMY) && !compiledchunk.isEmpty()) {
				++i;
			}
		}
	...
	public void setupTerrain(final Entity viewEntity, final double partialTicks, ICamera camera, final int frameCount, final boolean playerSpectator) {
		...
		final BlockPos blockpos1 = new BlockPos(d3, d4 + viewEntity.getEyeHeight(), d5);
		final ModRenderChunk renderchunk = this.viewFrustum.getRenderChunk(blockpos1);
		final BlockPos blockpos = new BlockPos(MathHelper.floor(d3 / 16.0D) * 16, MathHelper.floor(d4 / 16.0D) * 16, MathHelper.floor(d5 / 16.0D) * 16);
		...
		if (!flag && this.displayListEntitiesDirty) {
			...
			this.renderInfos = Lists.<ModRenderGlobal.ContainerLocalRenderInformation>newArrayList();
			final Queue<ModRenderGlobal.ContainerLocalRenderInformation> queue = Queues.<ModRenderGlobal.ContainerLocalRenderInformation>newArrayDeque();
			Entity.setRenderDistanceWeight(MathHelper.clamp(this.mc.gameSettings.renderDistanceChunks / 8.0D, 1.0D, 2.5D));
			boolean flag1 = this.mc.renderChunksMany;

			if (renderchunk != null) {
				boolean flag2 = false;
				final ModRenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation3 = new ModRenderGlobal.ContainerLocalRenderInformation(renderchunk, (EnumFacing) null, 0);
				...
				for (int j = -this.renderDistanceChunks; j <= this.renderDistanceChunks; ++j) {
					for (int k = -this.renderDistanceChunks; k <= this.renderDistanceChunks; ++k) {
						final ModRenderChunk renderchunk1 = this.viewFrustum.getRenderChunk(new BlockPos((j << 4) + 8, i, (k << 4) + 8));
						...
						if ((renderchunk1 != null) && camera.isBoundingBoxInFrustum(renderchunk1.boundingBox.expand(0.0, blockpos1.getY() > 0 ? Double.POSITIVE_INFINITY : Double.NEGATIVE_INFINITY, 0.0))) // Forge: fix MC-73139
						{
							...
							queue.add(new ModRenderGlobal.ContainerLocalRenderInformation(renderchunk1, (EnumFacing) null, 0));
						}
					}
				}
			}
			...
			while (!queue.isEmpty()) {
				final ModRenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation1 = queue.poll();
				final ModRenderChunk renderchunk3 = renderglobal$containerlocalrenderinformation1.renderChunk;
				...
				for (final EnumFacing enumfacing1 : EnumFacing.values()) {
					final ModRenderChunk renderchunk2 = this.getRenderChunkOffset(blockpos, renderchunk3, enumfacing1);
					...
					if ((!flag1 || !renderglobal$containerlocalrenderinformation1.hasDirection(enumfacing1.getOpposite())) && (!flag1 || (enumfacing2 == null) || renderchunk3.getCompiledChunk().isVisible(enumfacing2.getOpposite(), enumfacing1)) && (renderchunk2 != null) && renderchunk2.setFrameIndex(frameCount) && camera.isBoundingBoxInFrustum(renderchunk2.boundingBox)) {
						final ModRenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation = new ModRenderGlobal.ContainerLocalRenderInformation(renderchunk2, enumfacing1, renderglobal$containerlocalrenderinformation1.counter + 1);
		...
		final Set<ModRenderChunk> set = this.chunksToUpdate;
		this.chunksToUpdate = Sets.<ModRenderChunk>newLinkedHashSet();

		for (final ModRenderGlobal.ContainerLocalRenderInformation renderglobal$containerlocalrenderinformation2 : this.renderInfos) {
			final ModRenderChunk renderchunk4 = renderglobal$containerlocalrenderinformation2.renderChunk;

			if (renderchunk4.needsUpdate() || set.contains(renderchunk4)) {
				this.displayListEntitiesDirty = true;
				final BlockPos blockpos2 = renderchunk4.getPosition().add(8, 8, 8);
				final boolean flag3 = blockpos2.distanceSq(blockpos1) < 768.0D;
	 ...
	 */
	@Nullable
	public ModRenderChunk getRenderChunkOffset(final BlockPos playerPos, final ModRenderChunk renderChunkBase, final EnumFacing facing) {
	...
	public void fixTerrainFrustum(final double x, final double y, final double z) {
	...
	@Override
	public Vector3f getViewVector(final Entity entityIn, final double partialTicks) {
	...
	public int renderBlockLayer(final BlockRenderLayer blockLayerIn, final double partialTicks, final int pass, final Entity entityIn) {
		...
		final RenderBlockLayerEvent layerEvent = NoCubesHooksClient.onRenderBlockLayerEvent(this, blockLayerIn, partialTicks, pass, entityIn, 0);
		if (layerEvent.isCanceled()) {
			return layerEvent.getChunksRendered();
		}
		...
		final int renderInfosSizeMinusOne = flag ? this.renderInfos.size() - 1 : 0;
		final int renderInfosSize = flag ? -1 : this.renderInfos.size();
		final int stepSize = flag ? -1 : 1;

		for (int j = renderInfosSizeMinusOne; j != renderInfosSize; j += stepSize) {
			final ModRenderChunk renderchunk = (this.renderInfos.get(j)).renderChunk;
	...
	public void renderBlockLayer(final BlockRenderLayer blockLayerIn) {
		this.mc.entityRenderer.enableLightmap();
	...
	public void cleanupDamagedBlocks(final Iterator<DestroyBlockProgress> iteratorIn) {
	...
	public void renderSky(final float partialTicks, final int pass) {
		final net.minecraftforge.client.IRenderHandler renderer = this.world.provider.getSkyRenderer();
		if (renderer != null) {
		...
		if (this.mc.world.provider.getDimensionType().getId() == 1) {
			...
			final Vec3d vec3d = this.world.getSkyColor(this.mc.getRenderViewEntity(), partialTicks);
			...
			tessellator.draw();
			GlStateManager.disableTexture2D();
			final float f15 = this.world.getStarBrightness(partialTicks) * f16;
	...
	@Override
	public void renderClouds(final float partialTicks, final int pass, final double p_180447_3_, final double p_180447_5_, final double p_180447_7_) {
		...
		if (this.mc.world.provider.isSurfaceWorld()) {
			if (this.mc.gameSettings.shouldRenderClouds() == 2) {
				...
				final float f9 = 4.8828125E-4F;
				final double d5 = this.cloudTickCounter + partialTicks;
				double d3 = p_180447_3_ + (d5 * 0.029999999329447746D);
				final int i2 = MathHelper.floor(d3 / 2048.0D);
				final int j2 = MathHelper.floor(p_180447_7_ / 2048.0D);
				d3 = d3 - (i2 * 2048);
				final double lvt_22_1_ = p_180447_7_ - (j2 * 2048);
				final float f6 = (this.world.provider.getCloudHeight() - (float) p_180447_5_) + 0.33F;
				final float f7 = (float) (d3 * 4.8828125E-4D);
				final float f8 = (float) (lvt_22_1_ * 4.8828125E-4D);
				bufferbuilder.begin(7, DefaultVertexFormats.POSITION_TEX_COLOR);
	...
	public void renderCloudsFancy(final float partialTicks, final int pass, final double x, final double y, final double z) {
		GlStateManager.disableCull();
		final Tessellator tessellator = Tessellator.getInstance();
		final BufferBuilder bufferbuilder = tessellator.getBuffer();
		final float f = 12.0F;
		final float f1 = 4.0F;
		final double d3 = this.cloudTickCounter + partialTicks;
		double d4 = (x + (d3 * 0.029999999329447746D)) / 12.0D;
		double d5 = (z / 12.0D) + 0.33000001311302185D;
		final float f2 = (this.world.provider.getCloudHeight() - (float) y) + 0.33F;
		final int k1 = MathHelper.floor(d4 / 2048.0D);
		final int l1 = MathHelper.floor(d5 / 2048.0D);
		d4 = d4 - (k1 * 2048);
		d5 = d5 - (l1 * 2048);
		this.renderEngine.bindTexture(CLOUDS_TEXTURES);
		GlStateManager.enableBlend();
		...
		final float f25 = f3 * 0.9F;
		final float f26 = f4 * 0.9F;
		final float f27 = f5 * 0.9F;
		final float f9 = f3 * 0.7F;
		final float f10 = f4 * 0.7F;
		final float f11 = f5 * 0.7F;
		final float f12 = f3 * 0.8F;
		final float f13 = f4 * 0.8F;
		final float f14 = f5 * 0.8F;
		final float f15 = 0.00390625F;
		final float f16 = MathHelper.floor(d4) * 0.00390625F;
		final float f17 = MathHelper.floor(d5) * 0.00390625F;
		final float f18 = (float) (d4 - MathHelper.floor(d4));
		final float f19 = (float) (d5 - MathHelper.floor(d5));
		final int i2 = 8;
		final int j2 = 4;
		final float f20 = 9.765625E-4F;
		...
		for (int k2 = 0; k2 < 2; ++k2) {
			...
			for (int l2 = -3; l2 <= 4; ++l2) {
				for (int i3 = -3; i3 <= 4; ++i3) {
					...
					final float f21 = l2 * 8;
					final float f22 = i3 * 8;
					final float f23 = f21 - f18;
	...
	public void updateChunks(final long finishTimeNano) {
		...
		if (!this.chunksToUpdate.isEmpty()) {
			final Iterator<ModRenderChunk> iterator = this.chunksToUpdate.iterator();

			while (iterator.hasNext()) {
				final ModRenderChunk renderchunk1 = iterator.next();
				boolean flag1;
				...
				iterator.remove();
				final long k1 = finishTimeNano - System.nanoTime();
	...
	public void renderWorldBorder(final Entity entityIn, final float partialTicks) {
		...
		if ((entityIn.posX >= (worldborder.maxX() - d3)) || (entityIn.posX <= (worldborder.minX() + d3)) || (entityIn.posZ >= (worldborder.maxZ() - d3)) || (entityIn.posZ <= (worldborder.minZ() + d3))) {
			...
			final float f = ((k1 >> 16) & 255) / 255.0F;
			final float f1 = ((k1 >> 8) & 255) / 255.0F;
			final float f2 = (k1 & 255) / 255.0F;
			...
			if (d5 > (worldborder.maxX() - d3)) {
				...
				for (double d10 = d8; d10 < d9; f7 += 0.5F) {
					final double d11 = Math.min(1.0D, d9 - d10);
					final float f8 = (float) d11 * 0.5F;
			...
			if (d5 < (worldborder.minX() + d3)) {
				...
				for (double d12 = d8; d12 < d9; f9 += 0.5F) {
					final double d15 = Math.min(1.0D, d9 - d12);
					final float f12 = (float) d15 * 0.5F;
			...
			if (d7 > (worldborder.maxZ() - d3)) {
				...
				for (double d13 = d8; d13 < d9; f10 += 0.5F) {
					final double d16 = Math.min(1.0D, d9 - d13);
					final float f13 = (float) d16 * 0.5F;
			...
			if (d7 < (worldborder.minZ() + d3)) {
				...
				for (double d14 = d8; d14 < d9; f11 += 0.5F) {
					final double d17 = Math.min(1.0D, d9 - d14);
					final float f14 = (float) d17 * 0.5F;
	...
	public void drawBlockDamageTexture(final Tessellator tessellatorIn, final BufferBuilder bufferBuilderIn, final Entity entityIn, final float partialTicks) {
		...
		if (!this.damagedBlocks.isEmpty()) {
			...
			while (iterator.hasNext()) {
				...
				final TileEntity te = this.world.getTileEntity(blockpos);
	...
	public void playRecord(@Nullable final SoundEvent soundIn, final BlockPos pos) {
		...
		if (soundIn != null) {
			...
			final ISound positionedsoundrecord = PositionedSoundRecord.getRecordSoundRecord(soundIn, pos.getX(), pos.getY(), pos.getZ());
	...
	public void setPartying(final World p_193054_1_, final BlockPos pos, final boolean p_193054_3_) {
		for (final EntityLivingBase entitylivingbase : p_193054_1_.getEntitiesWithinAABB(EntityLivingBase.class, (new AxisAlignedBB(pos)).grow(3.0D))) {
	...
	public void spawnParticle(final EnumParticleTypes particleIn, final double xCoord, final double yCoord, final double zCoord, final double xSpeed, final double ySpeed, final double zSpeed, final int... parameters) {
	...
	@Nullable
	public Particle spawnParticle0(final int particleID, final boolean ignoreRange, final double xCoord, final double yCoord, final double zCoord, final double xSpeed, final double ySpeed, final double zSpeed, final int... parameters) {
		return this.spawnParticle0(particleID, ignoreRange, false, xCoord, yCoord, zCoord, xSpeed, ySpeed, zSpeed, parameters);
	}

	@Nullable
	public Particle spawnParticle0(final int particleID, final boolean ignoreRange, final boolean minParticles, final double xCoord, final double yCoord, final double zCoord, final double xSpeed, final double ySpeed, final double zSpeed, final int... parameters) {
		final Entity entity = this.mc.getRenderViewEntity();
	...
	public int calculateParticleLevel(final boolean p_190572_1_) {
	...
	public void playEvent(final EntityPlayer player, final int type, final BlockPos blockPosIn, final int data) {
		...
		case 1037:
			...
			break;
		case 2000:
			final int j2 = (data % 3) - 1;
			final int k1 = ((data / 3) % 3) - 1;
			final double d11 = blockPosIn.getX() + (j2 * 0.6D) + 0.5D;
			final double d13 = blockPosIn.getY() + 0.5D;
			final double d15 = blockPosIn.getZ() + (k1 * 0.6D) + 0.5D;

			for (int l2 = 0; l2 < 10; ++l2) {
				final double d16 = (random.nextDouble() * 0.2D) + 0.01D;
				final double d19 = d11 + (j2 * 0.01D) + ((random.nextDouble() - 0.5D) * k1 * 0.5D);
				final double d22 = d13 + ((random.nextDouble() - 0.5D) * 0.5D);
				final double d25 = d15 + (k1 * 0.01D) + ((random.nextDouble() - 0.5D) * j2 * 0.5D);
				final double d27 = (j2 * d16) + (random.nextGaussian() * 0.01D);
				final double d29 = -0.03D + (random.nextGaussian() * 0.01D);
				final double d30 = (k1 * d16) + (random.nextGaussian() * 0.01D);
		...
		case 2001:
			...
			if (block.getDefaultState().getMaterial() != Material.AIR) {
				final SoundType soundtype = block.getSoundType(Block.getStateById(data), this.world, blockPosIn, null);
		...
		case 2002:
		case 2007:
			final double d9 = blockPosIn.getX();
			final double d10 = blockPosIn.getY();
			final double d12 = blockPosIn.getZ();
			...
			final float f5 = ((data >> 16) & 255) / 255.0F;
			final float f = ((data >> 8) & 255) / 255.0F;
			final float f1 = ((data >> 0) & 255) / 255.0F;
			...
			for (int j3 = 0; j3 < 100; ++j3) {
				final double d18 = random.nextDouble() * 4.0D;
				final double d21 = random.nextDouble() * Math.PI * 2.0D;
				final double d24 = Math.cos(d21) * d18;
				final double d26 = 0.01D + (random.nextDouble() * 0.5D);
				final double d28 = Math.sin(d21) * d18;
				final Particle particle1 = this.spawnParticle0(enumparticletypes.getParticleID(), enumparticletypes.getShouldIgnoreRange(), d9 + (d24 * 0.1D), d10 + 0.3D, d12 + (d28 * 0.1D), d24, d26, d28);
			...
			break;
		case 2003:
			final double d3 = blockPosIn.getX() + 0.5D;
			final double d4 = blockPosIn.getY();
			final double d5 = blockPosIn.getZ() + 0.5D;
		...
		case 2004:
			...
			for (int i3 = 0; i3 < 20; ++i3) {
				final double d17 = blockPosIn.getX() + 0.5D + ((this.world.rand.nextFloat() - 0.5D) * 2.0D);
				final double d20 = blockPosIn.getY() + 0.5D + ((this.world.rand.nextFloat() - 0.5D) * 2.0D);
				final double d23 = blockPosIn.getZ() + 0.5D + ((this.world.rand.nextFloat() - 0.5D) * 2.0D);
		...
		case 2006:
			...
			for (int i2 = 0; i2 < 200; ++i2) {
				...
				final Particle particle = this.spawnParticle0(EnumParticleTypes.DRAGON_BREATH.getParticleID(), false, blockPosIn.getX() + (d6 * 0.1D), blockPosIn.getY() + 0.3D, blockPosIn.getZ() + (d8 * 0.1D), d6, d7, d8);
	...
	public class ContainerLocalRenderInformation {
		final ModRenderChunk	renderChunk;
		final EnumFacing		facing;
		byte					setFacing;
		final int				counter;

		public ContainerLocalRenderInformation(final ModRenderChunk renderChunkIn, final EnumFacing facingIn, @Nullable final int counterIn) {
