...
//	}
//
//	private static double collide(AABB aabb, LevelReader world, double motion, CollisionContext ctx, AxisCycle cycle, Stream<VoxelShape> nonBlockShapes) {
//		if (aabb.getXsize() < 0.000001 || aabb.getYsize() < 0.000001 || aabb.getZsize() < 0.000001)
//			return motion;
//		if (Math.abs(motion) < 0.0000001)
//			return 0.0D;
//
//		var inverseCycle = cycle.inverse();
//		var cycledX = inverseCycle.cycle(Direction.Axis.X);
//		var cycledY = inverseCycle.cycle(Direction.Axis.Y);
//		var cycledZ = inverseCycle.cycle(Direction.Axis.Z);
//		var pos = new BlockPos.MutableBlockPos();
//
//		int minX = Mth.floor(aabb.min(cycledX) - 0.0000001) - 1;
//		int maxX = Mth.floor(aabb.max(cycledX) + 0.0000001) + 1;
//		int minY = Mth.floor(aabb.min(cycledY) - 0.0000001) - 1;
//		int maxY = Mth.floor(aabb.max(cycledY) + 0.0000001) + 1;
//		double d0 = aabb.min(cycledZ) - 0.0000001;
//		double d1 = aabb.max(cycledZ) + 0.0000001;
//		boolean motionInitiallyPositive = motion > 0.0D;
//		int minZ = motionInitiallyPositive ? Mth.floor(aabb.max(cycledZ) - 0.0000001) - 1 : Mth.floor(aabb.min(cycledZ) + 0.0000001) + 1;
...
//						continue;
//
//					pos.set(inverseCycle, x, y, z);
//					BlockState blockstate = world.getBlockState(pos);
//					if ((boundariesTouched != 1 || blockstate.hasLargeCollisionShape()) && (boundariesTouched != 2 || blockstate.is(Blocks.MOVING_PISTON))) {
